...
public class Effect
{
	...
	public Effect(int id, Char c, int layer, int loop, int loopCount, sbyte isStand)
	{
		...
		if (M387(id) == null)
		{
			...
			if (MyStream.M1110(text) != null)
			{
				...
				t.img = GameCanvas.M503("/effectdata/" + id + "/img.png");
			}
			else
			{
				Service.M1594().M1653((short)id);
			}
			M386(t);
		}
		indexFrom = -1;
	...
	public static void M385(int id)
	{
		...
		{
			EffectData t = (EffectData)vEffData.M1114(i);
			if (t.ID == id)
			{
				vEffData.M1119(t);
				break;
	...
	public static void M386(EffectData eff)
	{
		vEffData.M1111(eff);
		if (TileMap.mapID != 130 && vEffData.M1113() > 10)
		{
			for (int i = 0; i < 5; i++)
			{
				vEffData.M1118(0);
	...
	public static EffectData M387(int id)
	{
		for (int i = 0; i < vEffData.M1113(); i++)
		{
			EffectData t = (EffectData)vEffData.M1114(i);
	...
	public void M390(mGraphics g, int xLayer, int yLayer)
	{
			...
			else
			{
				...
				if (M387(effId).arrFrame != null)
				{
					if (!isGetTime)
					{
						...
						if (typeEff == 0)
						{
							t = Res.M1522(indexFrom, indexTo);
						}
					}
					switch (typeEff)
					{
					case 0:
						if (Res.M1530(x - 50, y - 50, 100, 100, Char.M113().cx, Char.M113().cy) && t > indexFrom && t < indexTo)
						{
							if (t < indexTo)
							{
								t = indexTo;
					...
					{
						SoundMn.M1878(x, y, SoundMn.FIREWORK, SoundMn.volume);
					}
