public class Scroll
{
	...
	public ScrollResult M1562(bool isGetSelectNow)
	{
		if (styleUPDOWN)
		{
	...
	private ScrollResult M1563(bool isGetNow)
	{
		int num = xPos;
		int num2 = yPos;
		int w = width;
		...
		{
			if (!pointerIsDowning && GameCanvas.M516(num, num2, w, h))
			{
	...
	private ScrollResult M1564()
	{
		...
		if (GameCanvas.isPointerDown)
		{
			if (!pointerIsDowning && GameCanvas.M516(num, y, w, h))
			{
				for (int i = 0; i < pointerDownLastX.Length; i++)
				{
	...
	public void M1566(int nItem, int ITEM_SIZE, int xPos, int yPos, int width, int height, bool styleUPDOWN, int ITEM_PER_LINE)
	{
		...
		Res.M1513("nItem= " + nItem + " ITEMSIZE= " + ITEM_SIZE + " heghit= " + height);
		if (styleUPDOWN)
		{
