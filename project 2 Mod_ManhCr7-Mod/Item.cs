...
public class Item
{
	...
	public string M797()
	{
		...
		{
			string[] array = itemOption.Where((ItemOption opt) => opt.optionTemplate.name.Contains("Sức đánh+#%") || opt.optionTemplate.name.Contains("HP+#%") || opt.optionTemplate.name.Contains("KI+#%") || opt.optionTemplate.name.Contains("Biến #% tấn công thành HP") || opt.optionTemplate.name.Contains("Biến #% tấn công thành KI") || opt.optionTemplate.name.Contains("+#% tiềm năng, sức mạnh") || opt.optionTemplate.name.Contains("Phản #% sát thương") || opt.optionTemplate.name.Contains("+#% vàng rơi")).Select(delegate(ItemOption opt)
			{
	...
	public void M798()
	{
		compare = GameCanvas.panel.M1365(this);
	}
	...
	public void M800(int x, int y, int upgrade, mGraphics g)
	{
		...
		{
			int num4 = x - num / 2 + M802(GameCanvas.gameTick - i * 4);
			int num5 = y - num / 2 + M801(GameCanvas.gameTick - i * 4);
		...
		if (upgrade == 4 || upgrade == 8)
		{
			...
			{
				int num6 = x - num / 2 + M802(GameCanvas.gameTick - num * 2 - j * 4);
				int num7 = y - num / 2 + M801(GameCanvas.gameTick - num * 2 - j * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8)
		{
			...
			{
				int num8 = x - num / 2 + M802(GameCanvas.gameTick - num * 2 - k * 4);
				int num9 = y - num / 2 + M801(GameCanvas.gameTick - num * 2 - k * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8 && upgrade != 12 && upgrade != 2 && upgrade != 5 && upgrade != 9)
		{
			...
			{
				int num10 = x - num / 2 + M802(GameCanvas.gameTick - num - l * 4);
				int num11 = y - num / 2 + M801(GameCanvas.gameTick - num - l * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8 && upgrade != 12 && upgrade != 2 && upgrade != 5 && upgrade != 9 && upgrade != 13 && upgrade != 3 && upgrade != 6 && upgrade != 10 && upgrade != 15)
		{
			...
			{
				int num12 = x - num / 2 + M802(GameCanvas.gameTick - num * 3 - m * 4);
				int num13 = y - num / 2 + M801(GameCanvas.gameTick - num * 3 - m * 4);
	...
	public Item M804()
	{
		...
		if (options != null)
		{
			...
			{
				ItemOption t2 = new ItemOption();
				t2.optionTemplate = ((ItemOption)options.M1114(i)).optionTemplate;
				t2.param = ((ItemOption)options.M1114(i)).param;
				t.options.M1111(t2);
			}
		}
		t.itemId = itemId;
	...
	public void M818(mGraphics g, int x, int y, int index)
	{
		if (index > 7)
		{
			index = 7;
		...
		if (index < 4)
		{
			...
			default:
			{
				for (int k = 0; k < 2; k++)
				{
					...
					{
						int num3 = x - 17 + M819(GameCanvas.gameTick + array3[k] - l * 4);
						int num4 = y - 12 + M820(GameCanvas.gameTick + array4[k] - l * 4);
				...
				for (int m = 2; m < 4; m++)
				{
					...
					{
						int num5 = x - 17 + M819(GameCanvas.gameTick + array3[m] - n * 4);
						int num6 = y - 12 + M820(GameCanvas.gameTick + array4[m] - n * 4);
			...
			case 1:
			{
				for (int i = 0; i < 2; i++)
				{
					...
					{
						int num = x - 17 + M819(GameCanvas.gameTick + array3[i] - j * 4);
						int num2 = y - 12 + M820(GameCanvas.gameTick + array4[i] - j * 4);
		...
		for (int num7 = 0; num7 < 4; num7++)
		{
			...
			{
				int num9 = x - 17 + M819(GameCanvas.gameTick + array3[num7] - num8 * 4);
				int num10 = y - 12 + M820(GameCanvas.gameTick + array4[num7] - num8 * 4);
