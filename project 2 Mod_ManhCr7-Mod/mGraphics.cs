...
public class mGraphics
{
	...
	public void M936(string s, int x, int y, GUIStyle style)
	{
		...
		GUI.Label(new Rect(x - num, y - num2, ScaleUI.WIDTH, 100f), s, style);
		if (isClip)
		{
			GUI.EndGroup();
		}
	...
	public void M940(Image arg0, int x0, int y0, int w0, int h0, int arg5, int x, int y, int arg8)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		x *= zoomLevel;
		y *= zoomLevel;
	...
	public void M941(Image arg0, int x0, int y0, int w0, int h0, int arg5, float x, float y, int arg8)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		x *= (float)zoomLevel;
		y *= (float)zoomLevel;
	...
	public void M942(Image arg0, int x0, int y0, int w0, int h0, int arg5, int x, int y, int arg8, bool isClip)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		M940(arg0, x0, y0, w0, h0, arg5, x, y, arg8);
	}

	public void M943(Image image, int x0, int y0, int w, int h, int transform, float x, float y, int anchor)
	{
		if (image == null)
		{
			return;
	...
	public void M944(Image image, float x0, float y0, int w, int h, int transform, int x, int y, int anchor)
	{
		if (image == null)
		{
			return;
	...
	public void M945(Image image, float x0, float y0, int w, int h, int transform, float x, float y, int anchor)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
	...
	public void M946(Image image, float x0, float y0, int w, int h, int transform, int x, int y, int anchor)
	{
		...
		if (texture2D == null)
		{
			texture2D = Image.M705(image, (int)x0, (int)y0, w, h, transform).texture;
			M917(key, texture2D);
		}
		int num = 0;
		int num2 = 0;
	...
	public void M947(Image image, float x, float y)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		x *= (float)zoomLevel;
		y *= (float)zoomLevel;
	...
	public void M948(Image image, int x, int y, int anchor)
	{
		if (image != null)
		{
	...
	public void M949(Image image, int x, int y, int anchor)
	{
		if (image != null)
		{
	...
	public void M950(Image image, int x, int y)
	{
		if (image != null)
		{
	...
	public void M951(Image image, float x, float y, int anchor)
	{
		if (image != null)
		{
	...
	public void M956(Image image, int x, int y, int w, int h, int tranform)
	{
		GUI.color = Color.red;
		x *= zoomLevel;
		y *= zoomLevel;
	...
	public void M957(Image image, int x, int y)
	{
		x *= zoomLevel;
		y *= zoomLevel;
		if (image != null)
	...
	public static int M958(Image image)
	{
		return image.M727();
	}

	public static int M959(Image image)
	{
		return image.M728();
	}
	...
	public static Image M961(Image img0, float level, int rgb)
	{
		int num = rgb & 0xFF;
		int num2 = (rgb >> 8) & 0xFF;
		int num3 = (rgb >> 16) & 0xFF;
		...
		Image t = Image.M706(img0.M732(), img0.M733());
		t.texture.SetPixels(pixels);
		Image.M730(t.texture);
		t.texture.Apply();
		Cout.M329("BLEND ----------------------------------------------------");
		return t;
	}
	...
	public void M963(Image imgTrans, int x, int y, int w, int h)
	{
		M937(0, 0.5f);
	...
	public static int M966(Image img)
	{
		return img.w;
	}

	public static int M967(Image img)
	{
		return img.h;
	}
	...
	public void M970(MyVector totalLine)
	{
		...
		{
			mLine t = (mLine)totalLine.M1114(i);
	...
	public void M971(mGraphics g, int x, int y, int xTo, int yTo, int nLine, int color)
	{
		MyVector t = new MyVector();
		for (int i = 0; i < nLine; i++)
		{
			t.M1111(new mLine(x, y, xTo + i, yTo + i, color));
		}
		g.M970(t);
	}

	internal void M972(Small img, int p1, int p2, int p3, int p4, int transform, int x, int y, int anchor)
	{
