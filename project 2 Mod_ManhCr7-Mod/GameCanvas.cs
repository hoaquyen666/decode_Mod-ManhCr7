...
public class GameCanvas : IActionListener
{
	...
	public static ServerScreen serverScr;

	public bool resetToLoginScr;

	public GameCanvas()
	{
		switch (Rms.M1542("languageVersion"))
		{
		default:
			Main.main.doClearRMS();
			Rms.M1543("languageVersion", 2);
			break;
		case -1:
			Rms.M1543("languageVersion", 2);
			break;
		...
		clearOldData = Rms.M1542(GameMidlet.VERSION);
		if (clearOldData != 1)
		{
			Main.main.doClearRMS();
			Rms.M1543(GameMidlet.VERSION, 1);
		}
		M441();
	}
	...
	public void M441()
	{
		MotherCanvas.instance.M1024(this);
		w = MotherCanvas.instance.M1031();
		h = MotherCanvas.instance.M1032();
		hw = w / 2;
		hh = h / 2;
		isTouch = true;
		if (w >= 240)
		...
		instance = this;
		mFont.M891();
		mScreen.ITEM_HEIGHT = mFont.tahoma_8b.M912() + 8;
		M443();
		M513();
		M507();
		panel = new Panel();
		imgShuriken = M503("/mainImage/myTexture2df.png");
		int num = Rms.M1542("clienttype");
		if (num != -1)
		{
			if (num > 7)
			{
				Rms.M1543("clienttype", mSystem.clientType);
			}
			else
			{
		...
		if (mSystem.clientType == 7 && (Rms.M1536("fake") == null || Rms.M1536("fake") == string.Empty))
		{
		...
		borderConnerW = mGraphics.M958(imgBorder[0]);
		borderConnerH = mGraphics.M959(imgBorder[0]);
		borderCenterW = mGraphics.M958(imgBorder[1]);
		borderCenterH = mGraphics.M959(imgBorder[1]);
		Panel.graphics = Rms.M1542("lowGraphic");
		lowGraphic = Rms.M1542("lowGraphic") == 1;
		GameScr.isPaintChatVip = Rms.M1542("serverchat") != 1;
		Char.isPaintAura = Rms.M1542("isPaintAura") == 1;
		Res.M1506();
		SmallImage.M1779();
		...
		InfoMe.M754().M755();
		Command.btn0left = M503("/mainImage/btn0left.png");
		Command.btn0mid = M503("/mainImage/btn0mid.png");
		Command.btn0right = M503("/mainImage/btn0right.png");
		Command.btn1left = M503("/mainImage/btn1left.png");
		Command.btn1mid = M503("/mainImage/btn1mid.png");
		Command.btn1right = M503("/mainImage/btn1right.png");
		...
		ServerListScreen.M1578();
		serverScr = new ServerScreen();
	}

	public static GameCanvas M442()
	{
		return instance;
	}
	...
	public void M445()
	{
		...
		try
		{
			...
			if (gameTick > 10000)
			{
				if (mSystem.M1054() - lastTimePress > 20000L && currentScreen == loginScr)
				{
			...
			if (Controller.isConnectOK)
			{
				if (Controller.isMain)
				{
					...
					Cout.M326("Connect ok");
					ServerListScreen.testConnect = 2;
					Rms.M1543("svselect", ServerListScreen.ipSelect);
					Rms.M1553(GameMidlet.IP + ":" + GameMidlet.PORT);
					Service.M1594().M1630();
					Service.M1594().M1596();
				}
				else
	...
	public void M446()
	{
		...
		isResume = true;
		Session_ME.M1744().M1743();
		Session_ME2.M1757().M1756();
	...
	public void M447()
	{
		...
		{
			M494(mResources.lost_connection + LoginScr.serverName, 888395, null);
		}
		else
		{
			M488();
	...
	public static void M449()
	{
		...
		{
			Session_ME.M1744().connect(GameMidlet.IP, GameMidlet.PORT);
		}
	}

	public static void M450()
	{
		if (!Session_ME2.M1757().isConnected())
		{
			Res.M1513("IP2= " + GameMidlet.IP2 + " PORT 2= " + GameMidlet.PORT2);
			Session_ME2.M1757().connect(GameMidlet.IP2, GameMidlet.PORT2);
		}
	}

	public static void M451(mGraphics g)
	{
	...
	public void M457(mScreen screen)
	{
		try
		{
			SoundMn.M1826().M1873();
			...
			GameScr.textTime.M1120();
			GameScr.vClan.M1120();
			GameScr.vFriend.M1120();
			GameScr.vEnemies.M1120();
		...
		{
			Cout.M326("Loi tai doResetToLoginScr " + ex.ToString());
		}
	}
	...
	public static void M459(mGraphics g)
	{
	}
	...
	public static void M461(mGraphics g, int color, int x, int y, int w, int h, int detalY)
	{
		g.M933(color);
		int cmy = GameScr.cmy;
		if (cmy > GameCanvas.h)
	...
	public static void M462(mGraphics g, int layer, int deltaY, int color1, int color2)
	{
		try
		{
			int num = layer - 1;
			if (num == imgBG.Length - 1 && (GameScr.M559().isRongThanXuatHien || GameScr.M559().isFireWorks))
			{
				g.M933(GameScr.M559().mautroi);
				g.M932(0, 0, w, h);
				...
				if (GameScr.M559().isFireWorks && !lowGraphic)
				{
					FireWorkEff.M433(g);
				}
			}
			else
			{
				...
				if (color1 != -1)
				{
					if (num == nBg - 1)
					{
						M461(g, color1, 0, -(cmy >> deltaY), GameScr.gW, yb[num], deltaY);
					}
					else
					{
						M461(g, color1, 0, yb[num - 1] + bgH[num - 1], GameScr.gW, yb[num] - (yb[num - 1] + bgH[num - 1]), deltaY);
					}
				}
				if (color2 != -1)
				{
					if (num == 0)
					{
						M461(g, color2, 0, yb[num] + bgH[num], GameScr.gW, GameScr.gH - (yb[num] + bgH[num]), deltaY);
					}
					else
					{
						M461(g, color2, 0, yb[num] + bgH[num], GameScr.gW, yb[num - 1] - (yb[num] + bgH[num]) + 80, deltaY);
					}
				}
				if (currentScreen == GameScr.instance)
				{
					...
					{
						g.M948(imgSun2, -(GameScr.cmx >> layerSpeed[0]) + 400, yb[0] + 30 - (cmy >> 2), StaticObj.BOTTOM_HCENTER);
					}
					if (layer == 1 && typeBg == 13)
					{
						g.M948(imgBG[1], -(GameScr.cmx >> layerSpeed[0]) + 200, yb[0] - (cmy >> 3) + 30, 0);
						g.M940(imgBG[1], 0, 0, bgW[1], bgH[1], 2, -(GameScr.cmx >> layerSpeed[0]) + 200 + bgW[1], yb[0] - (cmy >> 3) + 30, 0);
					}
					if (layer == 3 && TileMap.mapID == 1)
					{
						for (int k = 0; k < TileMap.pxh / mGraphics.M959(imgCaycot); k++)
						{
							g.M948(imgCaycot, -(GameScr.cmx >> layerSpeed[2]) + 300, k * mGraphics.M959(imgCaycot) - (cmy >> 3), 0);
						}
					}
				}
				EffectMn.M379(g, -(GameScr.cmx + moveX[num] >> layerSpeed[num]), yb[num] + bgH[num] - (cmy >> deltaY), num);
			}
		}
		catch (Exception ex)
		{
			Cout.M328("Loi ham paint bground: " + ex.ToString());
		}
	}

	public static void M463(mGraphics g)
	{
		if (imgSun != null)
		{
	...
	public static void M464(mGraphics g)
	{
		if (imgSun2 != null)
		{
	...
	public static void M466(mGraphics g)
	{
		if (!isLoadBGok)
		{
			g.M933(0);
	...
	public static void M469(int typeBG)
	{
		try
		{
			...
			if (typeBg == 12)
			{
				BackgroundEffect.yfog = TileMap.pxh - 100;
			}
			else
			{
				BackgroundEffect.yfog = TileMap.pxh - 160;
			}
			BackgroundEffect.M39();
			M470(typeBG);
			...
			else
			{
				try
				{
					...
					int[] data = new int[1];
					Image t = M502(path);
					t.M734(ref data, 0, 1, mGraphics.M966(t) / 2, 0, 1, 1);
					skyColor = data[0];
				}
				catch (Exception)
				{
			...
			if (TileMap.bgType == 100)
			{
				...
				for (int i = 0; i < nBg; i++)
				{
					...
					{
						int[] data2 = new int[1];
						imgBG[i].M734(ref data2, 0, 1, mGraphics.M966(imgBG[i]) / 2, 0, 1, 1);
						colorTop[i] = data2[0];
						data2 = new int[1];
						imgBG[i].M734(ref data2, 0, 1, mGraphics.M966(imgBG[i]) / 2, mGraphics.M967(imgBG[i]) - 1, 1, 1);
						colorBotton[i] = data2[0];
						bgW[i] = mGraphics.M958(imgBG[i]);
						bgH[i] = mGraphics.M959(imgBG[i]);
					}
					else if (nBg > 1)
					{
			...
			else
			{
				for (int j = 0; j < nBg; j++)
				{
					...
					{
						int[] data3 = new int[1];
						imgBG[j].M734(ref data3, 0, 1, mGraphics.M966(imgBG[j]) / 2, 0, 1, 1);
						colorTop[j] = data3[0];
						data3 = new int[1];
						imgBG[j].M734(ref data3, 0, 1, mGraphics.M966(imgBG[j]) / 2, mGraphics.M967(imgBG[j]) - 1, 1, 1);
						colorBotton[j] = data3[0];
						bgW[j] = mGraphics.M958(imgBG[j]);
						bgH[j] = mGraphics.M959(imgBG[j]);
					}
					else if (nBg > 1)
					{
			...
			if (typeBg != 0)
			{
				...
				{
					imgSun = M502("/bg/sun3" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					imgSun2 = M502("/bg/sun4" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
				...
				{
					imgSun = M502("/bg/sun5" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					imgSun2 = M502("/bg/sun6" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					sunX = GameScr.gW - GameScr.gW / 3;
					sunY = yb[4];
					sunX2 = sunX - 100;
					sunY2 = yb[4] + 20;
				...
				{
					imgSun = M502("/bg/sun8" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					imgSun2 = M502("/bg/sun7" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					sunX = GameScr.gW / 2 - 50;
					sunY = yb[3] + 20;
				...
				{
					imgSun = M502("/bg/sun9" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					imgSun2 = M502("/bg/sun10" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
					sunX = GameScr.gW / 2 - 30;
					sunY = yb[3] + 60;
					sunX2 = GameScr.gW / 2 + 20;
					sunY2 = yb[3] + 10;
				...
				else
				{
					...
					{
					case 9:
						imgSun = M502("/bg/sun11" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						imgSun2 = M502("/bg/sun12" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						sunX = GameScr.gW - GameScr.gW / 3;
						sunY = yb[4] + 20;
						sunX2 = sunX - 80;
						sunY2 = yb[4] + 40;
						break;
					case 10:
						imgSun = M502("/bg/sun13" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						imgSun2 = M502("/bg/sun14" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						sunX = GameScr.gW - GameScr.gW / 3;
						sunY = yb[1] - 30;
						sunX2 = sunX - 80;
						sunY2 = yb[1];
						break;
					case 11:
						imgSun = M502("/bg/sun15" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						imgSun2 = M502("/bg/b113" + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						sunX = GameScr.gW / 2 - 30;
						sunY = yb[2] - 30;
						break;
					case 12:
					...
					default:
						...
						imgSun = null;
						imgSun2 = null;
						imgSun = M502("/bg/sun" + typeBG + ((TileMap.bgType != 0) ? ("-" + TileMap.bgType) : string.Empty) + ".png");
						sunX = GameScr.gW - GameScr.gW / 3;
						sunY = yb[2] - 30;
						break;
					case 16:
	...
	private static void M470(int typeBG)
	{
		...
		while (true)
		{
			...
			{
				if (typeBG == bgRain[num] && Res.M1522(0, 2) == 0)
				{
					break;
				}
				num++;
		...
		BackgroundEffect.M55(0);
	}

	public void M471(int keyCode)
	{
		lastTimePress = mSystem.M1054();
	...
	public void M472(int keyCode)
	{
		...
		case -7:
			...
			break;
		case -4:
			if ((currentScreen is GameScr || currentScreen is CrackBallScr) && Char.M113().isAttack)
			{
				M484();
				M483();
			}
		...
		case 48:
			...
			break;
		case 49:
			if (currentScreen == CrackBallScr.instance || (currentScreen == GameScr.instance && isMoveNumberPad && !ChatTextField.M279().isShow))
			{
				keyHold[1] = true;
				keyPressed[1] = true;
			}
	...
	{
		if (Res.M1529(x - pxLast) >= 10 || Res.M1529(y - pyLast) >= 10)
		{
	...
	{
		return mSystem.M1054() - lastTimePress >= 800L;
	}
	...
	public static void M485()
	{
		...
		{
			M496(mResources.DOYOUWANTEXIT, new Command(mResources.YES, instance, 8885, null), new Command(mResources.NO, instance, 8882, null));
		}
	}

	public void M486(mGraphics g)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		M451(g);
		g.M933(0);
		g.M932(0, 0, w, h);
		g.M948(GameAutomationHub.logoServerListScreen, w / 2, h / 2 - 8, StaticObj.BOTTOM_HCENTER);
		M514(hw, h / 2 + 24, g);
		mFont.tahoma_7b_white.M898(g, mResources.PLEASEWAIT + ((LoginScr.timeLogin <= 0) ? string.Empty : (" " + LoginScr.timeLogin + "s")), w / 2, h / 2, 2);
	}

	public void M487(mGraphics gx)
	{
		try
		{
			debugPaint.M1120();
			...
			GameScr.info1.M756(g);
			GameScr.info2.M756(g);
			if (GameScr.M559().popUpYesNo != null)
			{
				GameScr.M559().popUpYesNo.M1488(g);
			}
			...
			for (int i = 0; i < Effect2.vEffect2.M1113(); i++)
			{
				...
				if (t is ChatPopup && !t.Equals(ChatPopup.currChatPopup) && !t.Equals(ChatPopup.serverChatPopUp))
				{
					t.paint(g);
				}
			}
	...
	public static void M489(string info)
	{
		...
		{
			M444();
			msgdlg.M1037(info, null, new Command(mResources.OK, instance, 8882, null), null);
	...
	{
		M444();
		msgdlg.M1037(info, null, new Command(mResources.CANCEL, instance, 8882, null), null);
		currentDialog = msgdlg;
		msgdlg.isWait = true;
	}
	...
	{
		msgdlg.M1037(str, new Command(strLeft, this, 8881, url), null, new Command(strRight, this, 8882, null));
		currentDialog = msgdlg;
	}
	...
	{
		M444();
		msgdlg.M1037(info, null, new Command(mResources.OK, instance, actionID, p), null);
		msgdlg.show();
	}
	...
	public static void M495(string info, int iYes, object pYes, int iNo, object pNo)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		M444();
		msgdlg.M1037(info, new Command(mResources.YES, instance, iYes, pYes), new Command(string.Empty, instance, iYes, pYes), new Command(mResources.NO, instance, iNo, pNo));
		msgdlg.show();
	}

	public static void M496(string info, Command cmdYes, Command cmdNo)
	{
		M444();
	...
	public static Image M502(string path)
	{
		...
		catch (Exception ex)
		{
			try
			{
				string[] array = Res.M1531(path, "/", 0);
				sbyte[] array2 = Rms.M1535("x" + mGraphics.zoomLevel + array[array.Length - 1]);
				if (array2 != null)
				{
					result = Image.M708(array2, 0, array2.Length);
					return result;
				}
				return result;
			}
			...
			{
				Cout.M328("Loi ham khong tim thay a: " + ex.ToString());
				return result;
			}
		}
	}

	public static Image M503(string path)
	{
		path = Main.res + "/x" + mGraphics.zoomLevel + path;
		path = M504(path);
		Image result = null;
	...
	public static void M514(int x, int y, mGraphics g)
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		g.M940(imgShuriken, 0, Main.f * 16, 16, 16, 0, x, y, mGraphics.HCENTER | mGraphics.VCENTER);
	}
	...
	public void perform(int idAction, object p)
	{
		...
		case 8887:
		{
			M488();
			int charId2 = (int)p;
			Service.M1594().M1682(charId2);
			break;
		}
		case 8888:
		{
			int charId = (int)p;
			Service.M1594().M1683(charId);
			M488();
			break;
		}
		case 8889:
		...
		case 88810:
		{
			...
			M488();
			Service.M1594().M1665(playerMapId);
			break;
		}
		case 88811:
			M488();
		...
		case 88814:
		{
			Item[] items = (Item[])p;
			M488();
			Service.M1594().M1664(items);
			break;
		}
		case 88817:
			ChatPopup.M271(string.Empty, 1, Char.M113().npcFocus);
			Service.M1594().M1657(Char.M113().npcFocus.template.npcTemplateId, menu.menuSelectedItem, 0);
			break;
		case 88818:
		{
			short menuId3 = (short)p;
			Service.M1594().M1659(menuId3, inputDlg.tfInput.M1924());
			M488();
			break;
		}
		case 88819:
		...
		case 88820:
		{
			...
			if (array.Length > 1)
			{
				...
				{
					t.M1111(new Command(array[i + 1], instance, 88821, menuSelectedItem));
				}
				menu.M877(t, 3);
			}
			else
			{
				ChatPopup.M271(string.Empty, 1, Char.M113().npcFocus);
				Service.M1594().M1657(Char.M113().npcFocus.template.npcTemplateId, menuSelectedItem, 0);
			}
			break;
		}
		case 88821:
		{
			int menuId = (int)p;
			ChatPopup.M271(string.Empty, 1, Char.M113().npcFocus);
			Service.M1594().M1657(Char.M113().npcFocus.template.npcTemplateId, menuId, menu.menuSelectedItem);
			break;
		}
		case 88822:
			ChatPopup.M271(string.Empty, 1, Char.M113().npcFocus);
		...
		case 88829:
		{
			...
			{
				Service.M1594().M1700(text4, (int)p);
				InfoDlg.M749();
			}
			break;
		}
		case 88836:
			...
			inputDlg.M777(mResources.INPUT_PRIVATE_PASS, new Command(mResources.ACCEPT, instance, 888361, null), TField.INPUT_TYPE_NUMERIC);
			break;
		case 88837:
		{
			...
			try
			{
				Service.M1594().M1708(int.Parse(text3.Trim()));
				break;
			}
			catch (Exception ex4)
			{
				Cout.M326("Loi tai 88837 " + ex4.ToString());
				break;
			}
		}
		case 88839:
		...
		case 888391:
		{
			string s = (string)p;
			M488();
			Service.M1594().M1706(int.Parse(s));
			break;
		}
		case 888392:
			Service.M1594().M1657(4, menu.menuSelectedItem, 0);
			break;
		case 888393:
			if (loginScr == null)
			{
		...
		case 888361:
		{
			...
			if (text.Length >= 6 && !text.Equals(string.Empty))
			{
				try
				{
					Service.M1594().M1705(int.Parse(text));
					break;
				}
				catch (Exception ex)
				{
					M489(mResources.ALERT_PRIVATE_PASS_2);
					Cout.M326("Loi tai 888361 Gamescavas " + ex.ToString());
					break;
				}
			}
			M489(mResources.ALERT_PRIVATE_PASS_1);
	...
	public static void M517()
	{
		...
		isPointerJustDown = false;
		isPointerJustRelease = false;
		GameScr.M559().lastSingleClick = 0L;
		GameScr.M559().isPointerDowning = false;
	}
