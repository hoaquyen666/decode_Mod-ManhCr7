...
public class BigBoss : Mob, IMapObject
{
	...
	public void M2242()
	{
		...
		{
			data.M396(patch);
			data.img = GameCanvas.M503("/effectdata/" + 100 + "/img.png");
		}
		catch (Exception)
		{
	...
	public void M2243()
	{
		...
		{
			data.M396(patch);
			data.img = GameCanvas.M503("/effectdata/" + 101 + "/img.png");
	...
	private void M2249(mGraphics g)
	{
	...
	public void M2259(Char[] cAttack, int[] dame, sbyte type)
	{
		charAttack = cAttack;
		dameHP = dame;
		this.type = type;
	...
	public void M2260()
	{
		if (type == 7)
		{
			...
			{
				ServerEffect.M1571(70, x + ((dir != 1) ? (-15) : 15), y - 40, 1);
			}
		}
		if (type == 0)
		{
			...
			if (tick == 8)
			{
				for (int i = 0; i < charAttack.Length; i++)
				{
					MonsterDart.M1017(x + ((dir != 1) ? (-45) : 45), y - 30, isBoss: true, dameHP[i], 0, charAttack[i], 24);
				}
			}
		}
		if (type == 1)
	...
	public void M2267(Mob mobToAttack)
	{
