...
public class TField : IActionListener
{
	...
	public void M1903()
	{
		...
		if (imgTf == null)
		{
			imgTf = GameCanvas.M503("/mainImage/myTexture2dtf.png");
		}
	}
	...
	public void M1908()
	{
		if (paintedText != null && mFont.tahoma_8b != null)
		{
			...
			if (offsetX < 0 && mFont.tahoma_8b.M909(paintedText) + offsetX < width - TEXT_GAP_X - 13 - typingModeAreaWidth)
			{
				offsetX = width - 10 - typingModeAreaWidth - mFont.tahoma_8b.M909(paintedText);
			}
			if (offsetX + mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos)) <= 0)
			{
				offsetX = -mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos));
				offsetX += 40;
			}
			else if (offsetX + mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos)) >= width - 12 - typingModeAreaWidth)
			{
				offsetX = width - 10 - typingModeAreaWidth - mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos)) - 2 * TEXT_GAP_X;
			}
			if (offsetX > 0)
			{
				offsetX = 0;
	...
	public void M1914(int index)
	{
		...
		int num = mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos));
		switch (index)
		{
		default:
		...
		case 1:
			...
			{
				offsetX -= mFont.tahoma_8b.M909(paintedText.Substring(caretPos - 1, 1));
			}
			break;
		case -1:
			if (num + offsetX < 15 && caretPos > 0 && caretPos < paintedText.Length)
			{
				offsetX += mFont.tahoma_8b.M909(paintedText.Substring(caretPos, 1));
			}
			break;
		}
		if (offsetX > 0)
		...
		{
			int num2 = mFont.tahoma_8b.M909(paintedText) - (width - 12);
			if (offsetX < -num2)
			{
				offsetX = -num2;
			}
	...
	public void M1915(mGraphics g, bool iss, int x, int y, int w, int h, int xText, int yText, string text, string info)
	{
		g.M933(0);
		if (iss)
		{
		...
		{
			mFont.tahoma_8b.M898(g, text, xText, yText, 0);
		}
		else if (info != null)
		{
			if (iss)
			{
				mFont.tahoma_7b_focus.M898(g, info, xText, yText, 0);
			}
			else
			{
				mFont.tahoma_7b_unfocus.M898(g, info, xText, yText, 0);
			}
		}
	}

	public void M1916(mGraphics g)
	{
		...
		M1915(g, flag, x, y - 1, width, height + 5, TEXT_GAP_X + offsetX + x + 1, y + (height - mFont.tahoma_8b.M912()) / 2 + 2, paintedText, name);
		...
		if (flag && isPaintMouse && isPaintCarret)
		{
			...
			{
				g.M933(7999781);
				g.M932(TEXT_GAP_X + 1 + offsetX + x + mFont.tahoma_8b.M909(paintedText.Substring(0, caretPos) + "a") - CARET_WIDTH - mFont.tahoma_8b.M909("a"), y + (height - CARET_HEIGHT) / 2 + 5, CARET_WIDTH, CARET_HEIGHT);
			}
			GameCanvas.M451(g);
			if (text != null && text.Length > 0 && GameCanvas.isTouch)
			{
				g.M948(GameCanvas.imgClear, x + width - 13, y + height / 2 + 3, mGraphics.VCENTER | mGraphics.HCENTER);
			}
		}
	}
	...
	public void M1921()
	{
		if (GameCanvas.M481(x + width - 20, y, 40, height))
		{
			M1905();
			isFocus = true;
		}
		else if (GameCanvas.M481(x, y, width - 20, height))
		{
			M1923(isFocus: true);
		}
		else
	...
	public void M1923(bool isFocus)
	{
		...
		if (Thread.CurrentThread.Name == Main.mainThreadName && currentTField != null)
		{
			...
			kb = TouchScreenKeyboard.M1967(currentTField.text, t, b1: false, b2: false, type, b3: false, currentTField.name);
			if (kb != null)
			{
				kb.text = currentTField.text;
			}
			Cout.M331("SHOW KEYBOARD FOR " + currentTField.text);
		}
	}
