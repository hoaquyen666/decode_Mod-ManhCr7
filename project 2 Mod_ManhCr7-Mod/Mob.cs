...
public class Mob : IMapObject
{
	...
	static Mob()
	{
		...
		newMob = new MyVector();
		imgHP = GameCanvas.M503("/mainImage/myTexture2dmobHP.png");
	}
	...
	public void M975()
	{
		if (arrMobTemplate[templateId].data == null)
		{
			...
			if (MyStream.M1110(text) != null)
			{
				...
				arrMobTemplate[templateId].data.img = GameCanvas.M503(text + "/img.png");
			}
			else
			{
	...
	public void M983()
	{
		...
		{
			ServerEffect.M1573(114, this, 1);
		}
		if (typeSuperEff == 1 && GameCanvas.gameTick % 4 == 0)
		{
			ServerEffect.M1573(132, this, 1);
		}
		if (typeSuperEff == 2 && GameCanvas.gameTick % 7 == 0)
		{
			ServerEffect.M1573(131, this, 1);
		}
	}
	...
	public virtual void update()
	{
		...
		{
			ServerEffect.M1571(113, x, y, 1);
		}
		if (sleepEff && GameCanvas.gameTick % 10 == 0)
		{
			EffectMn.M376(new Effect(41, x, y, 3, 1, 1));
		}
		if (!GameCanvas.lowGraphic && status != 1 && status != 0 && !GameCanvas.lowGraphic && GameCanvas.gameTick % (15 + mobId * 2) == 0)
		{
			for (int i = 0; i < GameScr.vCharInMap.M1113(); i++)
			{
				...
				if (t != null && t.isFlyAndCharge && t.cf == 32)
				{
					...
					if (t.cgender == 0)
					{
						MonsterDart.M1017(x + dir * w, y, M1001(), -100, -100, t2, 25);
					}
				}
			}
			if (Char.M113().isFlyAndCharge && Char.M113().cf == 32)
			{
				Char t3 = new Char();
				t3.cx = Char.M113().cx;
				t3.cy = Char.M113().cy - Char.M113().ch;
				if (Char.M113().cgender == 0)
				{
					MonsterDart.M1017(x + dir * w, y, M1001(), -100, -100, t3, 25);
				}
			}
		}
		if (holdEffID != 0 && GameCanvas.gameTick % 5 == 0)
		{
			EffectMn.M376(new Effect(holdEffID, x, y + 24, 3, 5, 1));
		}
		if (isFreez)
		{
		...
		case 1:
			...
			if (isDie)
			{
				...
				if (isMobMe)
				{
					...
					{
						if (((Mob)GameScr.vMob.M1114(j)).mobId == mobId)
						{
							GameScr.vMob.M1118(j);
						}
					}
				}
				p1 = 0;
	...
	public void M984()
	{
		if (hp > 0 && status != 3)
		{
			timeStatus = 4;
			status = 7;
			if (M998().type != 0 && Res.M1529(x - xFirst) < 30)
			{
				x -= 10 * dir;
			}
		}
	...
	public static NewBoss M988(sbyte idBoss)
	{
		Mob t = (Mob)GameScr.vMob.M1114(idBoss);
		if (t is NewBoss)
		{
			return (NewBoss)t;
		}
	...
	public static void M989()
	{
		for (int i = 0; i < GameScr.vMob.M1113(); i++)
		{
			...
			if (t is BigBoss)
			{
				GameScr.vMob.M1119(t);
				break;
			}
		}
	}

	public void M990(Char cFocus)
	{
	...
	private void M995()
	{
		...
		case 5:
			p1++;
			if (p1 > mobId % 3 && (cFocus == null || Res.M1529(cFocus.cx - x) > 80) && (mobToAttack == null || Res.M1529(mobToAttack.x - x) > 80))
			{
				status = 5;
			}
			break;
		...
		case 3:
			p1++;
			if (p1 > 10 + mobId % 10 && (cFocus == null || Res.M1529(cFocus.cx - x) > 80) && (mobToAttack == null || Res.M1529(mobToAttack.x - x) > 80))
			{
				status = 5;
			}
			break;
	...
	public void M996()
	{
		...
		if (tick < array.Length)
		{
			...
			{
				SoundMn.M1826().M1831(isKick: false, 0.05f);
			}
		}
		if (p1 == 0)
		{
			...
			if (p2 > array.Length - 1 || p1 == 1)
			{
				...
				else
				{
					if (cFocus != null)
					{
						MonsterDart.M1017(x + dir * w, y, M1001(), dame, dameMp, cFocus, M998().dartType);
					}
					else
					{
						Char t = new Char();
						...
						t.cy = mobToAttack.y;
						t.charID = -100;
						MonsterDart.M1017(x + dir * w, y, M1001(), dame, dameMp, t, M998().dartType);
					}
					isBusyAttackSomeOne = false;
				}
			}
		...
		else if (p1 == 1)
		{
			...
			if (Res.M1529(xFirst - x) < 5 && Res.M1529(yFirst - y) < 5 && tick == array.Length)
			{
				status = 2;
				p1 = 0;
				p2 = 0;
	...
	public void M997()
	{
		...
		try
		{
			...
			if (!isDontMove && !isWind)
			{
				...
				case 3:
				{
					...
					if (Res.M1529(x - Char.M113().cx) < 40 && Res.M1529(x - xFirst) < arrMobTemplate[templateId].rangeMove)
					{
						dir = ((x <= Char.M113().cx) ? 1 : (-1));
						if (Res.M1529(x - Char.M113().cx) < 20)
						{
							x -= dir * 10;
						}
						status = 2;
		...
		{
			Cout.M326("lineee: " + num);
		}
	}

	public MobTemplate M998()
	{
	...
	public virtual void paint(mGraphics g)
	{
		...
		if (!M999() || (status == 1 && p3 > 0 && GameCanvas.gameTick % 3 == 0))
		{
			return;
		}
		...
		{
			SmallImage.M1785(g, smallBody, x, y + fy - 14, 0, 3);
		}
		g.M918(0, -GameCanvas.transY);
		if (Char.M113().mobFocus != null && Char.M113().mobFocus.Equals(this) && status != 1)
		{
	...
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		cTimeDie = mSystem.M1054();
		hp = 0;
		injureThenDie = true;
		hp = 0;
		status = 1;
		Res.M1513("MOB DIEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEe");
		p1 = -3;
		p2 = -dir;
		p3 = 0;
	}

	public void M1004(Mob mobToAttack)
	{
	...
	public void M1008()
	{
		...
		{
			frameArr = (int[][])Controller.frameHT_NEWBOSS.M1074(templateId + string.Empty);
			stand = frameArr[0];
			move = frameArr[1];
			moveFast = frameArr[2];
			attack1 = frameArr[3];
