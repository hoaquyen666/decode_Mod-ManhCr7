...
public class DataInputStream
{
	...
	{
		r = new myReader(ArrayCast.M0(((TextAsset)Resources.Load(filename, typeof(TextAsset))).bytes));
	}
	...
	public static DataInputStream M350(string filename)
	{
		return M352(filename);
	}

	private static DataInputStream M351(string filename)
	{
		if (status != 0)
		{
			for (int i = 0; i < 500; i++)
	...
	private static DataInputStream M352(string filename)
	{
		try
		{
