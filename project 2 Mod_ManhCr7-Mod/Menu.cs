public class Menu
{
	...
	{
		imgMenu1 = GameCanvas.M503("/mainImage/myTexture2dbtMenu1.png");
		imgMenu2 = GameCanvas.M503("/mainImage/myTexture2dbtMenu2.png");
	}

	public void M875(MyVector menuItems, int pos)
	{
		M877(menuItems, pos);
		disableClose = true;
	}

	public void M876(MyVector menuItems, int x, int y)
	{
		M877(menuItems, 0);
		menuX = x;
		menuY = y;
	...
	public void M877(MyVector menuItems, int pos)
	{
		if (showMenu)
		{
			return;
		...
		{
			menuSelectedItem = 0;
			Command t = (Command)menuItems.M1114(0);
		...
		if (menuItems.M1113() != 0)
		{
			...
			{
				Command t2 = (Command)menuItems.M1114(j);
				t2.isPlaySoundButton = false;
				mFont.tahoma_7_yellow.M909(t2.caption);
				t2.subCaption = mFont.tahoma_7_yellow.M907(t2.caption, menuW - 10);
				Res.M1513("c caption= " + t2.caption);
			}
			menuTemY = new int[menuItems.M1113()];
			menuX = (GameCanvas.w - menuItems.M1113() * menuW) / 2;
			if (menuX < 1)
			{
				menuX = 1;
			}
			...
			showMenu = true;
			menuSelectedItem = 0;
			cmxLim = this.menuItems.M1113() * menuW - GameCanvas.w;
			if (cmxLim < 0)
			{
				cmxLim = 0;
			}
	...
	public void M879()
	{
		if ((GameScr.M559().activeRongThan && GameScr.M559().isUseFreez) || !showMenu || M878())
		{
			return;
		}
		bool flag = false;
			...
			if (!GameCanvas.keyPressed[(!Main.isPC) ? 8 : 22] && !GameCanvas.keyPressed[(!Main.isPC) ? 6 : 24])
			{
				if (GameCanvas.keyPressed[(!Main.isPC) ? 5 : 25])
				{
					if (center != null)
					{
						...
						{
							if (center.actionListener == GameScr.M559())
							{
								GameScr.M559().M672(center.idAction, center.p);
							}
							else
							{
				...
				else if (GameCanvas.keyPressed[12] && !GameScr.M559().M616())
				{
					if (M878())
					{
						return;
				...
				else if (!GameScr.M559().M616() && !disableClose && (GameCanvas.keyPressed[13] || mScreen.M1034(right)))
				{
					if (M878())
					{
						return;
		...
		{
			if (!GameCanvas.M481(GameCanvas.panel.cp.cx, 0, GameCanvas.panel.cp.sayWidth + 2, GameCanvas.panel.cp.ch))
			{
				flag2 = true;
			}
			else
			{
				flag2 = false;
				GameCanvas.panel.cp.M274();
			}
		}
		if (!disableClose && GameCanvas.isPointerJustRelease && !GameCanvas.M516(menuX, menuY, w, menuH) && !pointerIsDowning && !GameScr.M559().M616() && flag2)
		{
			if (!M878())
			{
				pointerDownFirstX = 0;
				...
				GameCanvas.M517();
				Res.M1513("menu select= " + menuSelectedItem);
				isClose = true;
				close = true;
				SoundMn.M1826().M1856();
			}
		...
		{
			if (!pointerIsDowning && GameCanvas.M516(menuX, menuY, w, menuH))
			{
		...
		if (GameCanvas.isPointerJustRelease && pointerIsDowning)
		{
			...
			if (Res.M1529(i2) < 20 && Res.M1529(GameCanvas.px - pointerDownFirstX) < 20 && !isDownWhenRunning)
			{
				cmRun = 0;
				cmtoX = cmx;
				pointerDownFirstX = -1000;
	...
	public void M881(mGraphics g)
	{
		...
		for (int i = 0; i < menuItems.M1113(); i++)
		{
			...
			string[] array = ((Command)menuItems.M1114(i)).subCaption;
			if (array == null)
			{
				array = new string[1] { ((Command)menuItems.M1114(i)).caption };
			}
			...
			for (int j = 0; j < array.Length; j++)
			{
				if (i == menuSelectedItem)
				{
					mFont.tahoma_7b_green2.M898(g, array[j], menuX + i * menuW + menuW / 2, num + j * 14, 2);
				}
				else
				{
					mFont.tahoma_7b_dark.M898(g, array[j], menuX + i * menuW + menuW / 2, num + j * 14, 2);
				}
			}
		}
		g.M918(-g.M920(), -g.M921());
	...
	public void M882()
	{
		...
		else
		{
			...
			if (menuSelectedItem >= 0)
			{
				Command t = (Command)menuItems.M1114(menuSelectedItem);
				if (t != null)
				{
					SoundMn.M1826().M1856();
					t.M294();
	...
	public void M883()
	{
		...
		if (menuSelectedItem >= 0)
		{
			((Command)menuItems.M1114(menuSelectedItem))?.M294();
		}
	}
