...
public class RadarScr : mScreen
{
	...
	{
		TYPE_UI = true;
		Image img = mSystem.M1071("/radar/17.png");
		Image img2 = mSystem.M1071("/radar/3.png");
		Image img3 = mSystem.M1071("/radar/23.png");
		fraImgFocus = new FrameImage(img, 28, 28);
		fraImgFocusNone = new FrameImage(img2, 30, 30);
		fraEff = new FrameImage(img3, 11, 11);
		imgUI = mSystem.M1071("/radar/0.png");
		imgArrow_Left = mSystem.M1071("/radar/1.png");
		imgArrow_Right = mSystem.M1071("/radar/2.png");
		imgUIText = mSystem.M1071("/radar/17.png");
		imgArrow_Down = mSystem.M1071("/radar/4.png");
		imgLock = mSystem.M1071("/radar/5.png");
		imgUse_0 = mSystem.M1071("/radar/6.png");
		imgRank = new Image[7];
		for (int i = 0; i < 7; i++)
		{
			imgRank[i] = mSystem.M1071("/radar/" + (i + 7) + ".png");
		}
		imgUse = mSystem.M1071("/radar/14.png");
		imgBack = mSystem.M1071("/radar/15.png");
		imgChange = mSystem.M1071("/radar/16.png");
		imgUIText = mSystem.M1071("/radar/18.png");
		imgBar_1 = mSystem.M1071("/radar/19.png");
		imgPro_0 = mSystem.M1071("/radar/20.png");
		imgPro_1 = mSystem.M1071("/radar/21.png");
		imgBar_0 = mSystem.M1071("/radar/22.png");
		wUi = 200;
		hUi = 219;
	...
	public void M1495(MyVector list, int num, int numMax)
	{
		RadarScr.list = list;
		M1496(num, numMax);
		page = 1;
	...
	public static void M1497()
	{
		...
		{
			Info_RadaScr t = (Info_RadaScr)list.M1114(i);
			if (t != null && t.isUse == 1)
			{
				listUse.M1111(t);
			}
	...
	public void M1498()
	{
		...
		for (int i = num; i < num2; i++)
		{
			...
			Info_RadaScr t2 = (Info_RadaScr)t.M1114(i);
			if (t2 != null)
			{
				index[i - num] = t2.id;
			}
	...
	public override void update()
	{
		try
		{
			...
			focus_card = Info_RadaScr.M771(listUse, index[indexFocus]);
			if (TYPE_UI)
			{
				focus_card = Info_RadaScr.M771(list, index[indexFocus]);
			}
			GameScr.M559().update();
	...
	private void M1500()
	{
		if (GameCanvas.isPointerClick)
		{
			for (int i = 0; i < 5; i++)
			{
				if (GameCanvas.M481(xyItem[i][0], xyItem[i][1], 30, 30) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease && i != indexFocus)
				{
					M1503(i);
				}
			}
			if (GameCanvas.M481(xyArrow[0][0] - 5, xyArrow[0][1] - 5, 20, 20))
			{
			...
			if (GameCanvas.M481(xyArrow[2][0] - 5, xyArrow[2][1] - 5, 20, 20))
			{
				if (GameCanvas.isPointerDown)
				{
					dxArrow[1] = 1;
			...
			{
				if (GameCanvas.M481(xCmd[j] - 5, yCmd - 5, 20, 20))
				{
					if (GameCanvas.isPointerDown)
					{
						dxCmd[j] = 1;
		...
		if (!GameCanvas.M481(xText, 0, wText, yText + hText))
		{
			return;
		}
	...
	private void M1501(int i)
	{
		...
		case 1:
			if (focus_card != null)
			{
				Service.M1594().M1741(1, focus_card.id);
			}
			break;
		case 2:
	...
	public override void paint(mGraphics g)
	{
		try
		{
			...
			if (focus_card != null)
			{
				...
				GameScr.M631(g);
				mFont.tahoma_7b_yellow.M898(g, ((focus_card.level <= 0) ? " " : ("Lv." + focus_card.level + " ")) + focus_card.name, xUi + wUi / 2, yUi + 15, 2);
				mFont.tahoma_7_white.M898(g, "no." + focus_card.no, xUi + 30, yText - 2, 0);
			...
			for (int i = 0; i < index.Length; i++)
			{
				...
				Info_RadaScr t = Info_RadaScr.M771(listUse, index[i]);
				if (TYPE_UI)
				{
					t = Info_RadaScr.M771(list, index[i]);
				}
				if (t != null)
				{
					fraImgFocus.M439(t.rank, xyItem[i][0], xyItem[i][1] + num + num2, 0, 0, g);
					SmallImage.M1785(g, t.idIcon, xyItem[i][0] + 14, xyItem[i][1] + 14 + num + num2, 0, StaticObj.VCENTER_HCENTER);
