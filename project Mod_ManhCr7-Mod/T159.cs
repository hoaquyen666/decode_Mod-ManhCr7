...
public class T159
{
	...
	public static void M1793(T160.T222 ac)
	{
	}
	...
	{
		if (T51.isPlaySound && !(SoundRun == null))
		{
	...
	{
		if (T51.isPlaySound && !(SoundBGLoop == null))
		{
	...
	private static void M1817(string filename, int pos)
	{
		if (status != 0)
		{
			T24.M328("CANNOT LOAD AUDIO " + filename + " WHEN LOADING " + filenametemp);
			return;
		}
		...
		if (i == 100)
		{
			T24.M328("TOO LONG FOR LOAD AUDIO " + filename);
			return;
		}
		T24.M327("Load Audio " + filename + " done in " + i * 5 + "ms");
	}
