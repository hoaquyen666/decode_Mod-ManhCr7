...
public class T138
{
	...
	private static void M1539(string filename, sbyte[] data)
	{
		if (status != 0)
		{
			Debug.LogError("Cannot save RMS " + filename + " because current is saving " + T138.filename);
			return;
		}
		T138.filename = filename;
		T138.data = data;
		status = 2;
		int i;
		for (i = 0; i < 500; i++)
	...
	private static sbyte[] M1540(string filename)
	{
		if (status != 0)
		{
			Debug.LogError("Cannot load RMS " + filename + " because current is loading " + T138.filename);
			return null;
		}
		T138.filename = filename;
		data = null;
	...
	private static void M1545(string filename, sbyte[] data)
	{
		...
		fileStream.Write(T4.M1(data), 0, data.Length);
		fileStream.Flush();
		fileStream.Close();
	...
	public static void M1551(string name)
	{
		...
		{
			T24.M326("loi xoa RMS --------------------------" + ex.ToString());
		}
	}
